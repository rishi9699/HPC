...
Iterating over the sub-2d-array using a single variable, with OpenMP
*/
...
int main()
{
    ...
    float *pos1, *pos2; //To be used inside the loop during iteration
    float *temp=NULL;

    clock_t tic = clock();  //start counter
    ...
    while(counter > 0)
    {
        ...
        #pragma omp parallel for private(pos1, pos2)
